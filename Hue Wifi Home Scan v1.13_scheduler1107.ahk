...
/*
v1.13
	* initiate second check of the checking after 30 min.... however there's no chance to reset the loop need to trace back and figre out how to reset the loop... get rid of individual loops 
	* loooks like i solved it... have it doing the 30 minutes thing in each indivudal loop... commented out the secondcheck. 
v1.11 
...
If 0 > 0
	...
	FileAppend,%A_Now% Now starting %A_ScriptName%! mod 11/07/16 1037`nRunning on %A_ComputerName% from path %WHClocation%`n, %DaemonLog%
	Goto Checkcomp
	}
else
	{
	Msgbox, Must be run in command line with switches `n`n"Hue Wifi Home Scan v1.13_scheduler.ahk" [Number of IPs to check 1-3 ] [Phone1 IP] [Phone2 IP] [Phone3 IP] `n`n`This launches a program when your phones go in and out of wifi range. `ni.e. shut off the lights when no one is home. Turn the lights back on when someone comes back home. 
	ExitApp
	}
}
Return
...
Checkcomp:											;This is the actual main loop 
	...
	If Home_L_WIFI_State = 1
		...
		;if secondcheck < 2 
		;	{
		;	sleep 180000							;If this is your first check, wait 30 min and check again. 
		;	goto Checkcomp
		;	}
		;secondcheck = 0								; Reset secondcheck variable 
	...
	sleep 100 									;Since lights were turned off by wifi Check again every 100 ms 					
goto Checkcomp
;--------------------------------------------------------------------------------------
...
if compnumbers = 2
	...
	loop
		...
		if (bothcompsoff <= 1)
			sleep %detectedtimeout%								;Since a phone was detected and an action was performed start the loop again after 1 minutes. dont wanna kill phone with pings 
		else If (bothcompsoff >= 2 && bothcompsoff < 4)	
			sleep 180000
		else If (bothcompsoff >= 4)					;Break out of this ONLY when a phone hasn't been seen 4x i.e. it's still looping if it sees a phone
			break		
...
if compnumbers = 3
	...
	loop
		...
		if (bothcompsoff <= 2)
			sleep %detectedtimeout%								;Since a phone was detected and an action was performed start the loop again after 1 minutes. dont wanna kill phone with pings 
		else If (bothcompsoff > 2 && bothcompsoff < 6)					;Break out of this ONLY when a phone hasn't been seen 2x i.e. it's still looping if it sees a phone
			sleep 180000							; wait 30 minutes and check after every successful full batch.  	
