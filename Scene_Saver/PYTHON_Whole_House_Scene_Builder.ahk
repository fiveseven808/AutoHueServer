...
/*
0706 update
	- individual lights are now changed 5 at a time to increase speed
*/
...
IfExist, C:\Program Files\WinHue 3\whc.exe
 ...
 Loop
		...
		remainder := mod(a_index, 5)
		if remainder = 0
			waitnext = 1
			
		;msgbox, a_index = %a_index% and remainder = %remainder waitnext = %waitnext%
		If waitnext = 1
			{
			if C_mode1
				{
					...
					if C_mode1 = "ct"
						...
						FileAppend, runwait `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -sat:%sat1% -ct:%ct1% `n ,%Scenefile%
						waitnext = 0
						}					
					if C_mode1 = "xy"
						{
						...
						FileAppend, runwait `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -x:%x_cord1% -y:%y_cord1% -sat:%sat1% `n ,%Scenefile%
						waitnext = 0
						}
					if C_mode1 = "hs"
						{
						;msgbox, light at %a_index% is XY
						FileAppend, runwait `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -hue:%hue1% -sat:%sat1%`n ,%Scenefile%
						waitnext = 0
						}
				}
			else if hue1 > 1
				{
				;msgbox, hue1 = %hue1%
				FileAppend, runwait `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -hue:%hue1% -sat:%sat1% -ct:%ct1% `n ,%Scenefile%
				waitnext = 0
				}
			else if ct1 > 1
				{
				;msgbox, c1 = %ct1%
				FileAppend, runwait `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -ct:%ct1% `n ,%Scenefile%
				waitnext = 0
				}
			else if bri1 >= 1
				{
				;msgbox, else
				FileAppend, runwait `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% `n ,%Scenefile%
				waitnext = 0
				}
			}
		else if waitnext = 0 
			...
			if C_mode1
				{
					...
					if C_mode1 = "ct"
						{
						;msgbox, light at %a_index% is 
						FileAppend, run `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -sat:%sat1% -ct:%ct1% `n ,%Scenefile%
						}					
					if C_mode1 = "xy"
						{
						RegExMatch(xy1,"\[(.*)\,", x_cord)
						RegExMatch(xy1,"\,(.*)\]", y_cord)
						;msgbox, x= %x_cord1% and y = %y_cord1%
						FileAppend, run `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -x:%x_cord1% -y:%y_cord1% -sat:%sat1% `n ,%Scenefile%
						}
					if C_mode1 = "hs"
						{
						;msgbox, light at %a_index% is XY
						FileAppend, run `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -hue:%hue1% -sat:%sat1%`n ,%Scenefile%
						}
				}
			else if hue1 > 1
				{
				;msgbox, hue1 = %hue1%
				FileAppend, run `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -hue:%hue1% -sat:%sat1% -ct:%ct1% `n ,%Scenefile%
				}
			else if ct1 > 1
				{
				;msgbox, c1 = %ct1%
				FileAppend, run `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% -ct:%ct1% `n ,%Scenefile%
				}
			else if bri1 >= 1
				{
				;msgbox, else
				FileAppend, run `%WHClocation`% -l:%a_index% -on:%l_on1% -bri:%bri1% `n ,%Scenefile%
				}
			}
		}
